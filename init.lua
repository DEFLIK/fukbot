...
home = function(forcibly, interrupt) -- переход к начальной точке и сброс лута
            ...
            while true do
              ...
              if charge then
                if n_charge == max_charge then
                  ...
                  report('INFO: Инструмент заряжен на '..math.floor((n_charge+1)/max_charge*100)..'% ')
                end
  ...
  if enderchest and not forcibly then
    ...
    report('INFO: Пополнение заряда, ожидание...')
    while cur_energy < 0.98 do -- ждать полного заряда батареи
      ...
      if math.floor(prev_energy*100) >= math.floor(cur_energy*100) then
        ...
        report('WARN: Слишком малый запас энергии, чтобы продолжать работу')
      end
    end
  end
  ignore_check = nil
